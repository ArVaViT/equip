...
# a homoglyph typo.
"""Is what the publisher sent back actually a verse?
...
outside. Nine more rows carry the same thing, every one of them a Psalm.
...
вселенну``, ``слухати ме слова``, ``жити ме в кучках``, ``дожинати меш``,
``їсти мемо``, ``не мати му недостатку``. Twenty-odd verses spread across
Genesis, Exodus, Leviticus, Joshua, Acts and the Epistles, always after
an infinitive, always the same six endings: that is nineteenth-century
orthography applied consistently, not a space that fell into one word,
and a checker that "corrected" it would be overruling the edition rather
than repairing it. ``test_a_verse_with_a_space_through_a_word.py`` pins
this, and it is the test to read before widening anything here.

What is still missed, and why it is left missed
-----------------------------------------------
One of the ten: Psalm 121:1 comes back as ``О чі мої підношу на гори``.
``о`` is a real Ukrainian word — the same edition writes ``на молитву о
девятій годинї`` — so the stranded initial is indistinguishable from the
vocative in ``о Боже`` or ``о нерозумні Галати`` without a dictionary.
Refusing it would mean refusing those too. It is named in the PR so a
person can decide about it, which is the honest place for a judgement a
program cannot make.

What happens to a verse that fails
----------------------------------
...
translated placeholder left the reference standing over a hole (see
generation 3) — and nothing is repaired: joining ``Г`` to ``осподь`` is
obvious to a reader and unavailable to a program, which cannot tell it
apart from joining ``у`` to ``неї``. A layer whose reason for existing is
that it quotes Scripture exactly does not get to edit it.
"""
...
def _without_stress_marks(text: str) -> str:
    """Drop combining marks. The Russian edition prints stress on a name
    it expects to be misread — ``Дура́`` — and a combining acute is not a
    letter, so a tokenizer splits the word around it and invents a
    stranded letter that was never there."""
...
def malformed_fragment(text: str, locale: str) -> str | None:
    ...
    cleaned = _without_stress_marks(text)
    for match in _TOKEN.finditer(cleaned):
        token = match.group()
        if len(token) != 1 or _script_of(token) not in _KNOWN_SCRIPTS:
            continue
        letter = token.lower()
        stranded = letter not in words or (letter in _ENCLITIC and _opens_a_sentence(cleaned, match.start()))
        if stranded:
            return cleaned[max(0, match.start() - _CONTEXT) : match.end() + _CONTEXT].strip()
    return None


__all__ = ["malformed_fragment"]
