"""Every live row that names a person in a spelling their language does not print.

``validation._check_person_names`` runs on a translation as it is made.
...
questions. The same corpus ``audit_book_names`` reads, and for the same
reason: a defect in a soft-deleted chapter examines nobody.

Use
---
  python -m scripts.audit_person_names
  python -m scripts.audit_person_names --locale uk --full
...
def main() -> int:
    ...
    print(f"\n{flagged} of {read} live rows name a person their language does not print")
