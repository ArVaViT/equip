...
class TestGradeCsvExport:
    def test_completion_only_course_does_not_declare_anyone_passed(self, client: TestClient, db: Session):
        """The export must not award a pass the screen deliberately withholds.

        A course of reading chapters has nothing to grade, so every student
        lands in `completion_pass`. That is a fact about the course, not about
        the student — the gradebook renders a neutral «По завершению» there,
        with a comment saying a green «Зачёт» would award a pass to someone who
        never opened a chapter.

        The CSV is the more official of the two documents: it gets printed,
        filed and signed. It once wrote "Passed on completion" into every row,
        including students at 0% progress, and a teacher signing that sheet
        would be certifying something nobody verified.
        """
        course = _seed_course_direct(db, course_id="completion-course", title="Reading Only")
        student = User(
            id=uuid.uuid4(),
            email="never-opened@test.local",
            full_name="Never Opened",
        ...
        body = client.get(f"{GRADES_PREFIX}/course/completion-course/export-csv").text
