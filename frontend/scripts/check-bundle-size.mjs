...
const BUDGETS_GZIP_KB = {
  ...
  "three.module": 146, // WebGL for the two landing scenes. Larger than the
  //           shell, and deliberate: both scenes are `lazy()`-imported from
  //           PublicLanding alone, neither mounts under
  //           `prefers-reduced-motion` or without WebGL, and both stop
  //           rendering when they leave the viewport. A student opening a
  //           lesson never fetches a byte of it. If this name ever shows up
  //           in a route a signed-in user reaches, that is the bug — not the
  //           size. (It is `three.module` rather than a scene name because
  //           Rollup splits the shared dependency out once both scenes
  //           import it.)
