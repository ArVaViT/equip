...
### The legal documents are translated by hand, not by the pipeline

`backend/app/legal/documents/` holds four documents — `privacy`, `terms`,
`teacher-terms`, `providers` — in four languages, as sixteen Markdown
files. They are **not** UI strings and **not** course content, and neither
mechanism above applies to them:

- the parity guard does not see them (they are not in `locales/`);
- the course translation pipeline must not be pointed at them. It is tuned
  for lesson prose, it parks rows at `needs_review` for a person to look
  at, and a binding document that says something slightly different in
  Ukrainian because a model had an off day is worse than no translation.

What enforces them instead, in `backend/tests/`:

- `test_legal_registry.py::test_every_document_exists_in_all_four_languages`
  — a missing file is a failed build, not a fallback.
- `test_legal_registry.py::test_every_translation_has_the_same_shape_as_the_english`
  — headings, bullets, numbered items, table rows, bold markers and links
  must match the English file exactly. A translation that has one heading
  fewer has dropped a section.
- `test_a_changed_text_is_a_new_version.py` — every file is hashed against a
  fingerprint pinned in the registry, so a text cannot be edited under
  somebody who has already accepted it.

So a change to one of these documents is **four file edits, four new
fingerprints and a registry revision**, in one commit. English is the
source and, by the documents' own terms, the version that governs.
