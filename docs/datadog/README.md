...
| `equip.invitations.created_total` | `scope`, `role`, `kind` (`new`/`resend`) | — | `app/services/invitation_service.py::create_or_resend_invitation`. `kind` separates a second invitation from a second *send* of the same one — counting a resend as a new invitation halves the apparent acceptance rate |
| `equip.invitations.accepted_total` | `scope`, `role`, `role_changed`, `joined_organization` | — | `..._service.py::accept_invitation`, after the transaction commits. The two boolean tags say what the acceptance actually did: a teacher accepting a course invitation changes neither |
| `equip.invitations.refused_total` | `reason` (`expired`/`already_used`/`email_mismatch`/`lost_race`), `scope` | — | every refusal on the accept path. Without it "the link did not work" arrives as a message from a person, and an expired link, a revoked one and the wrong mailbox are indistinguishable |
